...
import processClassImport from '@salesforce/apex/ClassImportController.processClassImport';
...
import saveFieldMapping from '@salesforce/apex/ClassImportController.saveFieldMapping';
//import getFieldMapping from '@salesforce/apex/LATImportControllerNew.getFieldMapping';
import getTargetFieldMappings from '@salesforce/apex/ClassImportController.getTargetFieldMappings';
//import processLAT from '@salesforce/apex/ClassImportController.processClass';

export default class LatImportNew extends NavigationMixin(LightningElement) {
    @api recordId; // Mission record Id
    ...
    handleAutoMatch() {
        ...
        // First, create a direct mapping table for exact matches based on new column structure
        // const directMappings = {
        //     'DHS_Unique_Traveler_ID__c': ['DHS_Unique_Traveler_IDc', 'DHS Unique Traveler ID', 'Traveler ID', 'Unique ID'],
        //     'First_Name__c': ['FirstName', 'First Name', 'First_Name', 'FIRSTNAME'],
        //     'Middle_Name__c': ['MiddleName', 'Middle Name', 'Middle_Name', 'MIDDLENAME'],
        //     'Last_Name__c': ['LastName', 'Last Name', 'Last_Name', 'LASTNAME', 'Surname'],
        //     'Date_of_Birth__c': ['Birthdate', 'Birth Date', 'Date of Birth', 'DOB', 'BirthDate'],
        //     'Gender__c': ['GenderIdentity', 'Gender Identity', 'Gender', 'Sex'],
        //     'Nationality__c': ['Nationalityc', 'Nationality', 'Nation'],
        //     'Country_of_Citizenship__c': ['Country_of_Citizenshipc', 'Country of Citizenship', 'Citizenship', 'Citizen Country'],
        //     'Primary_Language__c': ['Primary_Languagec', 'Primary Language', 'Language', 'Native Language'],
        //     'Secondary_Language_s__c': ['Secondary_Language_sc', 'Secondary Language', 'Second Language', 'Languages'],
        //     'Email_Address__c': ['Email', 'Email Address', 'Contact Email', 'E-mail'],
        //     'U_S_Phone_Number__c': ['U.S. Phone Number', 'Phone', 'Phone Number', 'Contact Number', 'Mobile', 'Telephone'],
        //     'Emergency_Contact_Information__c': ['Emergency_Contact_Informationc', 'Emergency Contact Information', 'Emergency Contact', 'Emergency Info'],
        //     'Travel_Document_Type__c': ['Travel_Document_Typec', 'Travel Document Type', 'Document Type', 'Travel Doc Type'],
        //     'Travel_Document_Number__c': ['Travel_Document_Numberc', 'Travel Document Number', 'Document Number', 'Travel Doc Number', 'Passport Number'],
        //     'Issuing_Country__c': ['Issuing_Countryc', 'Issuing Country', 'Document Issuing Country', 'Country of Issue'],
        //     'Travel_Document_Expiration_Date__c': ['Travel_Document_Expiration_Datec', 'Travel Document Expiration Date', 'Document Expiry', 'Expiration Date'],
        //     'Travel_Document_Status__c': ['Travel_Document_Statusc', 'Travel Document Status', 'Document Status', 'Status'],
        //     'Photograph__c': ['Photographc', 'Photograph', 'Photo', 'Picture'],
        //     'Financial_Capability_Declaration__c': ['Financial_Capability_Declarationc', 'Financial Capability Declaration', 'Financial Declaration', 'Financial Capability'],
        //     'Preferred_Destination_Country__c': ['Preferred_Destination_Countryc', 'Preferred Destination Country', 'Destination Country', 'Preferred Country'],
        //     'Known_or_Alleged_Family_Relationships__c': ['Known_or_Alleged_Family_Relationshipsc', 'Known or Alleged Family Relationships', 'Family Relationships', 'Family Relations'],
        //     'Special_Needs_Medical_Conditions__c': ['Special Needs/Medical Conditions', 'Special Needs Medical Conditions', 'Medical Conditions', 'Special Needs'],
        //     'Stipend_Eligibility_Flag__c': ['Stipend_Eligibility_Flagc', 'Stipend Eligibility Flag', 'Stipend Eligibility', 'Eligible for Stipend'],
        //     'ICE_CBP_Screening_Status__c': ['ICE_CBP_Screening_Statusc', 'ICE CBP Screening Status', 'Screening Status', 'ICE Status'],
        //     'Routing_Recommendation__c': ['Routing_Recommendationc', 'Routing Recommendation', 'Routing', 'Recommended Route'],
        //     'Notes_Remarks__c': ['Notes_Remarksc', 'Notes Remarks', 'Notes', 'Remarks', 'Comments']
        // };
    ...
    handleSaveMapping() {
        // Validate that we have some mappings to save
        const mappingsCount = Object.keys(this.fieldMapping).length;
        
        if (mappingsCount === 0) {
            this.showToast('Warning', 'No field mappings to save', 'warning');
            return;
        }
        
        console.log('Saving field mapping:', this.fieldMapping);
        
        saveFieldMapping('DefaultMapping', this.fieldMapping)
            .then(() => {
                this.showToast('Success', `Field mapping saved successfully (${mappingsCount} mappings)`, 'success');
            })
            .catch(error => {
                console.error('Error saving field mapping:', error);
                this.showToast('Error', 'Error saving field mapping: ' + this.getErrorMessage(error), 'error');
            });
    }
    ...
    // handleProcessLATRecords() {
    //     // Process the LAT to create contacts and cases
    //     if (!this.importResults || !this.importResults.latId) {
    //         this.showToast('Error', 'No LAT record to process', 'error');
    //         return;
    //     }
        
    //     this.isProcessingLAT = true;
    //     this.processingResults = null;
        
    //     processLAT({ latId: this.importResults.latId })
    //         .then(result => {
    //             this.processingResults = result;
                
    //             if (result.success) {
    //                 this.showToast('Success', `LAT processed successfully. Created ${result.contactsCreated} contacts and ${result.casesCreated} cases.`, 'success');
    //             } else {
    //                 this.showToast('Warning', `LAT processed with ${result.errorCount} errors.`, 'warning');
    //             }
    //         })
    //         .catch(error => {
    //             console.error('Error processing LAT:', error);
    //             this.showToast('Error', 'Error processing LAT: ' + this.getErrorMessage(error), 'error');
    //         })
    //         .finally(() => {
    //             this.isProcessingLAT = false;
    //         });
    // }
